...
#define NUM 16
void NAME(convert_data_piece)(struct ThreadData *thread)
{
...
#if 0
void NAME(convert_data_pawn)(struct ThreadData *thread)
{
  int n = base_entry.num;
  uint8_t pos[MAX_PIECES];
  T *restrict src = convert_data.src;
...
void NAME(convert_est_data_piece)(struct ThreadData *thread)
{
  int i, j, m, p, q, r;
...
#if 0
void NAME(convert_est_data_pawn)(struct ThreadData *thread)
{
  int i, j, m, p, q, r;
  T *restrict table = est_data.table;
  int num_cands = est_data.num_cands;
