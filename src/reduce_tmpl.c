/*
  Copyright (c) 2018, 2024, 2025, 2026 Ronald de Man
...
void NAME(reconstruct_table_pass)(T *table, int stm, int k, T *v)
{
  FILE *F;
  char name[64];
  ...
  NAME(read_data_transform)(F, table, table_size, v);
...
#if 0
static void NAME(reconstruct_table)(T *table, int stm, struct dtz_map *map)
{
  int i, k;
  int num = map->max_num;
  uint16_t (*inv_map)[MAX_VALS] = map->inv_map;
...
void NAME(load_table)(T *table, int stm)
{
  FILE *F;
  char name[64];
